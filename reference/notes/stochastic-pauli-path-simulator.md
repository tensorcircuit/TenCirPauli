...
SPPS should be a separate stochastic `value_and_grad` engine, not a mode or threshold of deterministic weight-projected operator propagation. It can reuse Phase 3 infrastructure: parameter-slot GateTape, packed Pauli words, Clifford conjugation, Pauli-rotation commutation and branch rules, product-state expectation, reusable native handles, deterministic seed plumbing, and coarse Python-to-Rust calls. Its hot representation differs materially: each sample carries one Pauli word, accumulated importance weight, active branch factors, and gradient accumulation state rather than a hash map containing a dynamically expanded Pauli operator.
