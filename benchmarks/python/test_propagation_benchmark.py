...
def workload_12q() -> (
    tuple[advanced.GateTape, tcp.PauliOperator, np.ndarray, list[tuple[object, ...]]]
):
...
def test_propagation_engine_setup(benchmark: BenchmarkFixture) -> None:
    ...
    result = benchmark(advanced.PropagationEngine, tape, observable, max_weight=3)
...
def test_propagation_operator_materialization(benchmark: BenchmarkFixture) -> None:
    ...
    benchmark.extra_info["final_term_count"] = len(result.terms)
...
def test_matched_jax_reference_warm(benchmark: BenchmarkFixture) -> None:
    ...
    native = advanced.PropagationEngine(tape, observable, max_weight=3)
...
def test_clifford_heavy_100q_scalar(benchmark: BenchmarkFixture) -> None:
    ...
    benchmark.extra_info["final_term_count"] = len(engine.propagate_operator([]).terms)
...
def test_custom_ptm_sparse_and_dense_setup(benchmark: BenchmarkFixture) -> None:
    ...
    result = benchmark(advanced.PropagationEngine, tape, observable, max_weight=4)
