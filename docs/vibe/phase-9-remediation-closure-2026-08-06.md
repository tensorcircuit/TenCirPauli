...
S1 QWC reconstruction now validates binary samples, allocates the flat output, and runs the packed-mask parity loop inside one Rust helper called through `allow_threads`. Randomized tests compare every group against an independent NumPy oracle across single- and multiword qubit widths, a residency test corrupts `reconstruction_masks` without changing the result, and a large concurrent-observer test sees progress from another Python thread during reconstruction. The grouping benchmark records qubits, shots, group size, support density, group count, and output bytes separately from construction timing.
