...
同一 workload 的 TensorCircuit 1.8.0/JAX 0.10.0 complex128 对照显示：20q MVP 约为 `6.84/14.21 ms`（最近邻/次近邻），TenCirPauli native 约为 `5.70/13.37 ms`。此前的 16q `12.4/26.8 ms` 是 raw JAX BCOO construction，`207/414 ms` 是单独的 warm `sum_duplicates()`；这些数字不能直接作为完整矩阵接口比较。新增的同步配对记录 `20260801T120113Z_57fd4da-dirty` 将两侧都从公开 Python 调用计时到 sparse 对象完成：TenCirPauli `operator.coo()`/`.csr()` 返回 Python/NumPy arrays，TensorCircuit benchmark 调用 `PauliStringSum2COO()` 完成 raw BCOO 构造，再调用 `sum_duplicates()` 并同步 data/indices。中位数为 16q 最近邻 native COO/CSR `3.755/4.701 ms` 对 JAX `226.465 ms`，16q 最近加次近邻 `9.459/7.464 ms` 对 `458.715 ms`，20q 最近邻 `107.403/98.166 ms` 对 `5,613.480 ms`；因此按完整 canonical end-to-end 口径 native 约快 `48–61x`。MVP 的局域优势仍约为 `1.1–1.2x`，而 raw-only sparse construction 约为 `2.2–3.0x`；所有 JAX timed calls 都在内部同步。

20q 最近邻 sparse 也在本机 16 GiB policy 下直接测量：JAX raw BCOO warm construction 约 `304 ms`，共 `59,768,832` entries、data/indices 约 `1.91 GB`；warm `sum_duplicates()` 约 `5.25 s`，输出 padded `nse=20,971,520`、存储约 `671 MB`。其中只有 `11,010,048` 个值大于 `1e-12`，其余是约 `1.2e-16` 的 cancellation residual；TenCirPauli exact canonical COO/CSR 约 `113/102 ms`、`352/273 MB`。这组数字仍是 raw-only 与 canonicalization-only 的拆分证据；完整 Python-to-sparse 对齐结果以 `20260801T120113Z_57fd4da-dirty` 为准，不能把拆分结果相加后再与 native 结果重复比较。
