...
- Large 20-qubit evidence in the same clean label uses 64 full-width random terms for MVP and 3 full-width random terms for materialized sparse targets：native MVP plan construction/apply was approximately 0.084/8.06 ms, versus JAX first/warm MVP at 1.192 s/20.12 ms；native COO/CSR was approximately 85.15/87.05 ms, versus JAX raw BCOO first/warm construction at 259.6/17.87 ms and first/warm `sum_duplicates()` at 543.6/208.9 ms. The 20q/3-term sparse values and indices each occupy about 100.7 MB in COO; a 20q/64-term COO/CSR request is rejected before allocation because the estimated requests are about 4.29/3.77 GB against the 256 MiB default limit。
