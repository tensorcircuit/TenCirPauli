...
- P2 benchmark workloads：Rust Criterion 与 Python pytest-benchmark 已覆盖 1,000、10,000、100,000-term duplicate-heavy canonicalization；smoke harness 全部通过。历史 baseline 为 `p1-2e0f154`，最新 all-workload label 为 `20260801T101504Z_dc089491dd43`。
- P3 benchmark workloads：Rust Criterion 与 Python pytest-benchmark 已加入 QWC grouping（128/1,024 terms）；smoke harness 全部通过。历史 baseline 为 `p2-6b90270`，最新 all-workload label 为 `20260801T101504Z_dc089491dd43`。
- P4 benchmark workloads：Rust Criterion 与 Python pytest-benchmark 已加入 dense/COO/MVP/backend-plan construction/apply；smoke harness 全部通过。历史 baseline 为 `p3-acf5c60`，最新 all-workload label 为 `20260801T101504Z_dc089491dd43`。
...
- Final clean optimization evidence (`20260801T101504Z_dc089491dd43`)：release Rust Criterion measured `hamiltonian/scaling` at approximately 38.1 µs plan construction/67.2 µs one-shot MVP/7.32 µs reusable apply/138.9 µs COO/152.6 µs CSR for 10q/64 terms, and 1.52 ms plan construction/2.20 ms one-shot MVP/433.7 µs reusable apply for 16q/256 terms. Public Python warm reusable native medians were approximately 7.87 µs and 0.390 ms; public sparse COO medians were approximately 0.057/0.200/0.715 ms for 8q/32, 10q/64 and 12q/64。
- Same-workload TensorCircuit evidence in clean label `20260801T101504Z_dc089491dd43`：complex128 JAX warm MVP medians were approximately 31.5 µs and 2.28 ms for 10q/64 and 16q/256, versus TenCirPauli reusable native approximately 7.87 µs and 0.390 ms (about 4.0x and 5.8x faster). TensorCircuit NumPy sparse construction medians were approximately 3.51/9.20/20.91 ms on 8q/32, 10q/64 and 12q/64, versus TenCirPauli approximately 0.057/0.200/0.715 ms (about 62x/46x/29x faster). JAX BCOO warm sparse matvec medians were approximately 0.160/0.268 ms for 8q/32 and 10q/64。
- JAX sparse construction is now measured separately from matvec：first raw BCOO construction, including shape-specialized compile, was approximately 219/165/160 ms; warm raw construction was 0.794/1.337/1.457 ms; first `sum_duplicates()` was 453/427/450 ms; and warm `sum_duplicates()` was 1.668/4.981/20.739 ms for 8q/32, 10q/64 and 12q/64. TenCirPauli public COO is therefore approximately 14.0x/6.7x/2.0x faster than JAX raw warm construction, and approximately 29x/25x/29x faster than JAX warm duplicate canonicalization on these workloads。
...
2. Keep benchmark artifacts, local environments and machine-specific profile output untracked；the clean label is reproducible through `python benchmarks/run.py compare 20260801T101504Z_dc089491dd43` on this machine。

## Phase 1 completion record

- P0–P5 REQUIRED items and acceptance gates are implemented in local commits `829221e`, `2e0f154`, `6b90270`, `acf5c60`, `9c11117`, `c7d18c3`, `ff02ae8`, `4b10598` and `0a546a6`。
...
- Final clean benchmark label `20260801T101504Z_dc089491dd43`：Rust canonicalization 132.42 µs/1k, 1.3101 ms/10k, 13.135 ms/100k；QWC grouping 339.87 µs/128 and 21.895 ms/1024；dense/COO/MVP/backend-plan kernels 9.277/11.404/1.070/57.03 µs/ns；reusable plan construction/apply 38.07/7.32 µs at 10q/64 terms and 1.525/433.7 µs at 16q/256 terms。Python/TensorCircuit full-boundary medians are recorded above；numerical error remained within documented differential tolerances and benchmark results are informational, not CI gates。
