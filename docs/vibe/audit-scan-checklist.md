...
- **Over-defense / over-checking (do NOT file as actionable unless a representative workload triggers it).** Layered intermediate finiteness/overflow guards, repeated `is_finite` scans of already-validated data, NaN/inf rejection at magnitudes no real physics workload reaches (e.g. coefficient overflow near `f64::MAX`, `scale(f64::MAX)`), and "test that X never happens" monkeypatch/isinstance tests guarding hypothetical future refactors. The project explicitly prefers readability over defending implausible extremes; Phase 9 deliberately removed `checked_scale`/`checked_add`/per-term `is_finite` on bounded paths (trig-bounded propagation, near-`MAX` coefficient scaling). *Check before filing: would a real user on a representative scientific workload ever hit this? If only a contrived input (near-`MAX` magnitude, empty-then-huge, hypothetical future maintainer) triggers it, tag it `[OVER-DEFENSE]`, do NOT recommend re-implementation, and record it only so a future round does not re-raise it. Distinguish a boundary guard (one check at the I/O or function exit — acceptable) from layered intermediate scanning (reject).*
...
- A `[REAL]` / `[NARROW]` / `[OVER-DEFENSE]` classification. `[REAL]` = a representative scientific workload triggers it. `[NARROW]` = real but only an uncommon/edge workload triggers it. `[OVER-DEFENSE]` = only a contrived magnitude or a hypothetical future change triggers it; tag it, do NOT recommend re-implementation, and record it solely so a future round does not re-raise it. If a fix would add layered intermediate checks or hurt readability for no representative-workload benefit, explicitly mark it "do not implement."

### Worked example of the over-defense filter

A finder proposes: "operator aggregation no longer rejects `inf` coefficients; `scale(f64::MAX)` produces `inf` that propagates as `NaN`." Apply the filter: would a real physics workload ever scale a coefficient to ~1.8e308? No. The input boundary already rejects non-finite *inputs*. The intermediate `is_finite` scan that was removed existed only to catch a magnitude no representative workload reaches. Verdict: `[OVER-DEFENSE]`, do not re-implement, record only. Contrast: "SPPS path-product overflows to `NaN` at ~134 gates with default smoothing" — a long molecular-VQE SPPS circuit is a representative workload and the failure is *silent* (`Ok(NaN)`), so a *single exit* check (not layered intermediate scans) is `[NARROW]`-actionable.
