...
`quantum_chemistry_pyscf.py` is an optional PySCF example. It imports an RHF H2 Hamiltonian, maps it explicitly to Pauli form, and evaluates a fixed-parameter TensorCircuit state through the backend MVP plan. Install PySCF with `pip install "tencirpauli[chemistry]"` before running it.
